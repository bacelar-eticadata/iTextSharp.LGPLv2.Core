    ...
    public class BarcodeDatamatrix
    {
        ...
        public int Generate(byte[] text, int textOffset, int textSize)
        {
            ...
            if (Height == 0 || Width == 0)
            {
                ...
                e = getEncodation(text, textOffset + _extOut, textSize - _extOut, data, extCount, last.DataSize - extCount, Options, false);
                if (e < 0)
                {
            ...
            else
            {
                ...
                dm = _dmSizes[k];
                e = getEncodation(text, textOffset + _extOut, textSize - _extOut, data, extCount, dm.DataSize - extCount, Options, true);
                if (e < 0)
                {
                    return DM_ERROR_TEXT_TOO_BIG;
                }
            ...
            _place = Placement.DoPlacement(dm.height - (dm.height / dm.HeightSection * 2), dm.width - (dm.width / dm.WidthSection * 2));
        ...
        internal class Placement
        {
            ...
            {
                if (row < 0) { row += _nrow; col += 4 - ((_nrow + 4) % 8); }
