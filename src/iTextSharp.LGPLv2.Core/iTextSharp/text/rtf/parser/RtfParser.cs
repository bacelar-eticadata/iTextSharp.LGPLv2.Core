...
namespace iTextSharp.text.rtf.parser
{
    ...
    public class RtfParser
    {
        ...
        {
            return _currentState.Destination;
        }
        ...
        public int HandleCloseGroup()
        {
            ...
            {
                _currentState = (RtfParserState)_stackState.Pop();
            }
            else
            {
        ...
        {
            return (GetConversionType() == TYPE_CONVERT);
        }
        ...
        {
            return (GetConversionType() == TYPE_IMPORT_FRAGMENT);
        }
        ...
        {
            return (GetConversionType() == TYPE_IMPORT_FULL);
        }
        ...
        private void handleImportMappings(RtfImportMappings importMappings)
        {
            ...
            {
                _importMgr.ImportFont(fontNr, (string)importMappings.GetFontMappings()[fontNr]);
            }
            foreach (string colorNr in importMappings.GetColorMappings().Keys)
            {
                _importMgr.ImportColor(colorNr, (BaseColor)importMappings.GetColorMappings()[colorNr]);
            }
            foreach (string listNr in importMappings.GetListMappings().Keys)
            {
                _importMgr.ImportList(listNr, (string)importMappings.GetListMappings()[listNr]);
            }
            foreach (string stylesheetListNr in importMappings.GetStylesheetListMappings().Keys)
            {
                _importMgr.ImportStylesheetList(stylesheetListNr, (List)importMappings.GetStylesheetListMappings()[stylesheetListNr]);
            }
